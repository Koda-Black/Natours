...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // 2 Check if user exists && password is corrrect
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!currentUser) {
    next(
      new AppError(
        'The user account belonging to this token no longer exist.',
        401,
      ),
    );
  }
...
exports.isLoggedIn = async (req, res, next) => {
  // 1 Getting token from cookies if user is logged in
  try {
    if (req.cookies.jwt) {
      // 2 Verify token
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  // 3 Send it back to user as an email
  try {
